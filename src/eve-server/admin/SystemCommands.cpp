...
PyResult Command_hop(Client* pClient, CommandDB* db, EVEServiceManager &services, const Seperator& args) {
    if (!pClient->IsInSpace())
        throw CustomError("You must be in space to hop.");

    double distanceAu = 100.0;  // default hop distance if none provided
    if (args.argCount() >= 2) {
        if (!args.isNumber(1))
            throw CustomError("Correct Usage: /hop [distanceAU]");
        distanceAu = atof(args.arg(1).c_str());
    }

    if (distanceAu <= 0.0)
        throw CustomError("Hop distance must be greater than zero.");

    GPoint target(pClient->GetShipSE()->GetPosition());
    target.MakeRandomPointOnSphere(distanceAu * ONE_AU_IN_METERS);

    pClient->JumpOutEffect(pClient->GetSystemID());
    pClient->MoveToLocation(pClient->GetSystemID(), target);
    pClient->JumpInEffect();

    return new PyString("Hop complete.");
}
...
PyResult Command_sov(Client* pClient, CommandDB* db, EVEServiceManager &services, const Seperator& args) {
    if (args.argCount() != 3)
        throw CustomError("Correct Usage: /sov complete [itemID]");
    if (args.arg(1) != "complete")
        throw CustomError("Only '/sov complete [itemID]' is supported.");
    if (!args.isNumber(2))
        throw CustomError("Argument 2 should be a valid itemID.");

    uint32 itemID = atoi(args.arg(2).c_str());
    SystemEntity* pSE = pClient->SystemMgr()->GetSE(itemID);
    if ((pSE == nullptr) or !pSE->IsPOSSE())
        throw CustomError("Item %u is not a sovereignty/POS structure in this system.", itemID);

    StructureSE* pPOS = pSE->GetPOSSE();
    switch (pPOS->GetProcState()) {
        case EVEPOS::ProcState::Anchoring:
            pPOS->Anchor();
            return new PyString("Sovereignty structure anchoring completed.");
        case EVEPOS::ProcState::Onlining:
            pPOS->SetOnline();
            return new PyString("Sovereignty structure online transition completed.");
        case EVEPOS::ProcState::Offlining:
            pPOS->SetOffline();
            return new PyString("Sovereignty structure offline transition completed.");
        case EVEPOS::ProcState::Unanchoring:
            pPOS->PullAnchor();
            return new PyString("Sovereignty structure unanchoring transition completed.");
        default:
            return new PyString("No pending sovereignty transition to complete.");
    }
}
...
PyResult Command_pos(Client* pClient, CommandDB* db, EVEServiceManager &services, const Seperator& args) {
    if (args.argCount() != 3)
        throw CustomError("Correct Usage: /pos [online|offline|anchor|unanchor] [itemID]");

    const std::string action = args.arg(1);
    if (!args.isNumber(2))
        throw CustomError("Argument 2 should be a valid itemID.");
    ...
    SystemEntity* pSE = pClient->SystemMgr()->GetSE(itemID);
    if ((pSE == nullptr) or !pSE->IsPOSSE())
        throw CustomError("Item %u is not a POS structure in this system.", itemID);

    StructureSE* pPOS = pSE->GetPOSSE();
    if (action == "online") {
        pPOS->Online();
        return new PyString("POS online process started.");
    }
    if (action == "offline") {
        pPOS->Offline();
        return new PyString("POS offline process started.");
    }
    if (action == "anchor") {
        GPoint pos = pPOS->GetPosition();
        pPOS->SetAnchor(pClient, pos);
        return new PyString("POS anchoring process started.");
    }
    if ((action == "unanchor") or (action == "pullanchor")) {
        pPOS->PullAnchor();
        return new PyString("POS unanchoring process started.");
    }

    throw CustomError("Unsupported /pos action '%s'.", action.c_str());
}
