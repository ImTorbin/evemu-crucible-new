...
PyResult Command_warpto(Client* pClient, CommandDB* db, EVEServiceManager &services, const Seperator& args)
{
    ...
    if (args.argCount() != 2)
        throw CustomError("Correct Usage: /warpto [itemID]");
    if (!args.isNumber(1))
        throw CustomError("Argument 1 must be a valid itemID.");
    ...
    if (pSE == nullptr)
        throw CustomError("Unable to find item %u in your current system.", itemID);

    pClient->GetShipSE()->DestinyMgr()->WarpTo(pSE->GetPosition(), 0, false, pSE);

    char reply[64];
    snprintf(reply, 64, "Warp initiated to %s (%u).", pSE->GetName(), itemID);
    pClient->SendInfoModalMsg(reply);
    return new PyString(reply);
}
...
PyResult Command_dropLoot(Client* pClient, CommandDB* db, EVEServiceManager &services, const Seperator& args)
{
    ...
    if (!pClient->GetShipSE()->DestinyMgr())
        throw CustomError ("You have no destiny manager.");
    if ((args.argCount() != 2) or !args.isNumber(1))
        throw CustomError("Correct Usage: /dropLoot [bubbleID]");
