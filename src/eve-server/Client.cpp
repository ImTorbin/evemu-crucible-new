...
Client::~Client() {
    ...
    if (m_char.get() != nullptr) {   // we have valid character
        ...
        if (!sConsole.IsShutdown()) {
            // Always drop trade sessions for this character (same-station trades are dock-only,
            // but this clears indexes if state was left inconsistent and avoids leaking session structs).
    ...
    SafeDelete(pShipSE);
    SafeDelete(pSession);
    if (m_destinyEventQueue != nullptr)
        m_destinyEventQueue->clear();
    if (m_destinyUpdateQueue != nullptr)
