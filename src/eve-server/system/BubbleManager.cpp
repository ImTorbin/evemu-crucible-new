...
void BubbleManager::RemoveEmpty()
{
    ...
    while (itr != m_bubbles.end()) {
        if ((*itr)->IsEmpty()) {
            ...
            SystemBubble* deadBubble = *itr;
            itr = m_bubbles.erase(itr);
            RemoveBubble(deadBubble->GetSystem()->GetID(), deadBubble);
        } else {
            ++itr;
        }
    }
...
{
    std::vector<SystemBubble*> deadBubbles;
    auto range = m_sysBubbleMap.equal_range(systemID);
    for (auto itr = range.first; itr != range.second; ++itr) {
        if (itr->second != nullptr)
            deadBubbles.push_back(itr->second);
    }

    for (auto* bubble : deadBubbles) {
        m_bubbles.remove(bubble);
        RemoveBubble(systemID, bubble);
    }
}
