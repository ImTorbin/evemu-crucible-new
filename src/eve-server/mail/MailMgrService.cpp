...
bool PyListToMessageIDs(PyList* messageIDs, std::vector<int32>& out, const char* methodName)
{
    out.clear();
    out.reserve(messageIDs->size());

    PyList::const_iterator cur = messageIDs->begin();
    for (size_t index = 0; cur != messageIDs->end(); ++cur, ++index) {
        if (!(*cur)->IsInt()) {
...
PyResult MailMgrService::AssignLabels(PyCallArgs &call, PyList* messageIDs, PyInt* labelID)
{
    ...
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::AssignLabels"))
        return nullptr;

    m_db.ApplyLabels(call.client->GetCharacterID(), messageIds, labelID->value());
...
{
    std::vector<int32> messageIds;
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::DeleteMail"))
        return nullptr;

    for (int i = 0; i < messageIds.size(); i++) {
        int32 messageID = messageIds[i];
        m_db.DeleteMail(call.client->GetCharacterID(), messageID);
    }
...
{
    if (!isUnread->value()) {
        m_db.SetMailUnread(call.client->GetCharacterID(), messageID->value());
    } else {
        m_db.SetMailRead(call.client->GetCharacterID(), messageID->value());
    }
...
{
    std::vector<int32> messageIds;
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::GetMailHeaders"))
        return nullptr;

    std::ostringstream query;
    query << " SELECT m.messageID, m.senderID, m.toCharacterIDs, m.toListID, ";
    query << " m.toCorpOrAllianceID, m.title, m.sentDate";
    query << " FROM mailStatus AS s";
    query << " LEFT JOIN mailMessage AS m USING (messageID)";
    query << " WHERE s.characterID = " << call.client->GetCharacterID();
    if (messageIds.empty()) {
        query << " AND 1 = 0";
    } else {
        query << " AND (";
        query << " m.messageID = " << messageIds[0];
        for (size_t i = 1; i < messageIds.size(); ++i)
            query << " OR m.messageID = " << messageIds[i];
        query << ")";
    }
...
{
    std::vector<int32> messageIds;
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::MarkAsRead"))
        return nullptr;

    m_db.SetMailsRead(call.client->GetCharacterID(), messageIds);
...
{
    std::vector<int32> messageIds;
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::MarkAsUnread"))
        return nullptr;

    m_db.SetMailsUnread(call.client->GetCharacterID(), messageIds);
...
{
    std::vector<int32> messageIds;
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::MoveFromTrash"))
        return nullptr;

    m_db.RemoveStatusMasks(call.client->GetCharacterID(), messageIds, mailStatusMaskTrashed);
...
{
    std::vector<int32> messageIds;
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::MoveToTrash"))
        return nullptr;

    m_db.ApplyStatusMasks(call.client->GetCharacterID(), messageIds, mailStatusMaskTrashed);
...
{
    std::vector<int32> messageIds;
    if (!PyListToMessageIDs(messageIDs, messageIds, "MailMgrService::RemoveLabels"))
        return nullptr;

    m_db.RemoveLabels(call.client->GetCharacterID(), messageIds, labelID->value());
