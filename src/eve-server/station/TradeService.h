...
public:
    ...
    void RemoveActiveSession(uint32 contID);
    TradeSession* GetTradeSession(uint32 contID);
    TradeSession* GetTradeSessionForPair(uint32 myID, uint32 herID);
    ...
    std::map<uint32, ActiveSession> m_activeSessions;      // mapped as containerID, ActiveSession
    std::multimap<uint32, uint32> m_activeSessionIndex;    // mapped as characterID, containerID
...
public:
    TradeBound(EVEServiceManager& mgr, TradeService& parent, uint32 contID);
