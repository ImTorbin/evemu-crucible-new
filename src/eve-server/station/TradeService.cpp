...
TradeBound::TradeBound (EVEServiceManager& mgr, TradeService& parent, uint32 contID) :
...
PyResult TradeBound::OfferMoney(PyCallArgs &call, PyFloat* amount) {
    TradeSession* pTSes = m_TSvc->GetTradeSession(m_contID);
    if (pTSes == nullptr)
...
PyResult TradeBound::Abort(PyCallArgs &call) {
    ...
    if (pOther != nullptr)
        pOther->SendNotification("OnTrade", "charid", &tuple);
    m_TSvc->RemoveActiveSession(pTSes->m_tradeSession.containerID);
    // returns none
    return PyStatic.NewNone();
}
...
BoundDispatcher* TradeService::BindObject(Client* client, PyRep* bindParameters) {
    ...
    TradeSession* pTSes = GetTradeSessionForPair(args.myID, args.herID);
    uint32 contID = 0;
    if (pTSes == nullptr) {
        pTSes = new TradeSession();
        contID = GetTradeSessionID();
        ...
        cAS.contID = contID;
        cAS.ourTS = pTSes;
        m_activeSessions.emplace(contID, cAS);
        m_activeSessionIndex.emplace(args.myID, contID);
        m_activeSessionIndex.emplace(args.herID, contID);
    } else {
        contID = pTSes->m_tradeSession.containerID;
    }

    return new TradeBound(this->GetServiceManager(), *this, contID);
}
...
PyResult TradeService::InitiateTrade(PyCallArgs &call, PyInt* characterID) {
    ...
    if (target == nullptr) {
        call.client->SendErrorMsg("That character is not available for trading right now.");
        return nullptr;
    }
    ...
    if (GetTradeSessionForPair(call.client->GetCharacterID(), target->GetCharacterID()) != nullptr) {
        call.client->SendErrorMsg("You already have a trade window open with %s.", target->GetName());
        return nullptr;
    }

    if (!call.client->IsDocked() or !target->IsDocked() or (call.client->GetStationID() != target->GetStationID())) {
        call.client->SendErrorMsg("Both players must be docked in the same station to trade.");
        return nullptr;
    }
...
TradeSession* TradeService::GetTradeSession(uint32 contID) {
    auto itr = m_activeSessions.find(contID);
    if (itr == m_activeSessions.end())
        return nullptr;
    return itr->second.ourTS;
}

TradeSession* TradeService::GetTradeSessionForPair(uint32 myID, uint32 herID) {
    auto range = m_activeSessionIndex.equal_range(myID);
    for (auto itr = range.first; itr != range.second; ++itr) {
        auto sitr = m_activeSessions.find(itr->second);
        if (sitr == m_activeSessions.end())
            continue;
        const ActiveSession& ses = sitr->second;
        if (((ses.myID == myID) and (ses.herID == herID))
         or ((ses.myID == herID) and (ses.herID == myID)))
            return ses.ourTS;
    }
    return nullptr;
}

void TradeService::RemoveActiveSession(uint32 contID) {
    auto itr = m_activeSessions.find(contID);
    if (itr == m_activeSessions.end())
        return;

    const ActiveSession ses = itr->second;
    auto removeIndex = [this](uint32 charID, uint32 id) {
        auto range = m_activeSessionIndex.equal_range(charID);
        for (auto i = range.first; i != range.second; ++i) {
            if (i->second == id) {
                m_activeSessionIndex.erase(i);
                break;
            }
        }
    };
    removeIndex(ses.myID, contID);
    removeIndex(ses.herID, contID);
    SafeDelete(itr->second.ourTS);
...
void TradeService::CancelTrade(Client* pClient) {
    ...
    std::vector<uint32> sessions;
    auto range = m_activeSessionIndex.equal_range(pClient->GetCharacterID());
    for (auto itr = range.first; itr != range.second; ++itr)
        sessions.push_back(itr->second);

    for (auto contID : sessions) {
        auto sitr = m_activeSessions.find(contID);
        if (sitr == m_activeSessions.end())
            continue;

        TradeSession* pTSes = sitr->second.ourTS;
        Client* pA = sEntityList.FindClientByCharID(pTSes->m_tradeSession.myID);
        Client* pB = sEntityList.FindClientByCharID(pTSes->m_tradeSession.herID);

        TradeBound tb(this->GetServiceManager(), *this, contID);
        tb.CancelTrade(pA, pB, pTSes);
        ...
        PyIncRef(tuple);
        if (pA != nullptr)
            pA->SendNotification("OnTrade", "charid", &tuple);
        if (pB != nullptr)
            pB->SendNotification("OnTrade", "charid", &tuple);
        PySafeDecRef(tuple);
        RemoveActiveSession(contID);
    }
}
